...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Brevo (no domain purchase needed — just a verified single sender email).
    const bk = process.env.BREVO_API_KEY;
    if (!bk) return res.status(500).json({ error: "BREVO_API_KEY not set" });
    const senderEmail = process.env.SENDER_EMAIL;
    if (!senderEmail) return res.status(500).json({ error: "SENDER_EMAIL not set" });
    const senderName = process.env.SENDER_NAME || "Nexus AI";
    let sent = 0;
    const errors = [];
    for (const email of emails) {
      ...
      const payload = {
        ...
        to: [{ email }],
        subject: `Nexus AI — Daily Update (${dateStr})`,
        htmlContent: html,
      };
      if (link) payload.headers = { "List-Unsubscribe": `<${link}>` };
    ...
    return res.status(200).json({ ok: true, subscribers: emails.length, sent, commits: commits.length, errors });
  } catch (e) {
